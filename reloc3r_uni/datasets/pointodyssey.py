...
import baselines.DAS3R.dynamic_predictor.dust3r.utils.po_utils.basic
import baselines.DAS3R.dynamic_predictor.dust3r.utils.po_utils.improc
from baselines.DAS3R.dynamic_predictor.dust3r.utils.po_utils.misc import farthest_point_sample_py
from baselines.DAS3R.dynamic_predictor.dust3r.utils.po_utils.geom import apply_4x4_py, apply_pix_T_cam_py
import glob
import cv2
from torchvision.transforms import ColorJitter, GaussianBlur
from functools import partial
# from baselines.DAS3R.dynamic_predictor.dust3r.datasets.base.base_stereo_view_dataset import BaseStereoViewDataset, is_good_type, transpose_to_landscape
from baselines.DAS3R.dynamic_predictor.dust3r.datasets.base.base_stereo_view_dataset import is_good_type, transpose_to_landscape
from baselines.DAS3R.dynamic_predictor.dust3r.datasets.base.base_stereo_view_dataset import BaseStereoViewDataset 
#from baselines.reloc3r.reloc3r.datasets.base.base_stereo_view_dataset import BaseStereoViewDataset 

from baselines.DAS3R.dynamic_predictor.dust3r.utils.image import imread_cv2
from baselines.DAS3R.dynamic_predictor.dust3r.utils.misc import get_stride_distribution
from baselines.DAS3R.dynamic_predictor.dust3r.datasets.utils.geom import apply_4x4_py, realative_T_py
from baselines.DAS3R.dynamic_predictor.dust3r.utils.geometry import depthmap_to_absolute_camera_coordinates
...
class PointOdysseyDUSt3R(BaseStereoViewDataset):
    '''
    copy from das3r; reuse the dust3r in das3r
    
    '''
    def __init__(self,
                 dataset_location='/mnt/cluster/datasets/point_odyssey',
                 dset='train',
                 use_augs=False,
                 S=2,
                 N=16,
        ...
        if quick:
           ...
           self.sequences = ['/mnt/cluster/datasets/point_odyssey/test/seminar_g110_0315_3rd']
        #    self.sequences = ['/mnt/cluster/datasets/point_odyssey/test/egobody_egocentric']
            ...
            if os.path.isfile(info_path) and os.path.isfile(annotations_path):
                ...
                if len(traj_3d_files) and trajs_3d_shape > self.N:
                    ...
                    for stride in strides:
                        num = len(os.listdir(rgb_path))-self.S*max(stride,clip_step_last_skip)+1 
                        if self.quick:
                            num = 2
                        for ii in range(0, num, clip_step):
    ...
    def _get_views(self, index, resolution, rng):
        ...
        for i in range(2):
            ...
            camera_pose[:3,:3] = R.T
            camera_pose[:3,3] = -R.T @ t # camera pose world2cam
    ...
    def __getitem__(self, idx):
        ...
        for v, view in enumerate(views):
            ...
            pts3d, valid_mask = depthmap_to_absolute_camera_coordinates(**view,z_far=self.z_far)
...
if __name__ == "__main__":
    from baselines.DAS3R.dynamic_predictor.dust3r.datasets.base.base_stereo_view_dataset import view_name
    from baselines.DAS3R.dynamic_predictor.dust3r.viz import SceneViz, auto_cam_size
    from baselines.DAS3R.dynamic_predictor.dust3r.utils.image import rgb
    import gradio as gr
    import random
    ...
    dataset_location = '/mnt/cluster/datasets/point_odyssey'  # Change this to the correct path
    dset = 'train'
    dset = 'test'
    use_augs = False
    S = 2
