...
if __name__ == "__main__":
    # test transoformer_head with sample input
    config = {
        'd_model': 128,
        'px_resample_rule_dict_scale_step': {3: 8},
        'use_homogeneous_pose_t': 'True',
        'rotation_representation': '6D',
        'random_rescale_max': 10,
        'random_shift_max': 0,
        'num_resconv_block': 4,
        'transformer_pose_mean': torch.randn(1, 3, 1, 1)
    }

    transformer_head = Transformer_Head(config)
    input_sc = torch.randn(1, 3, 256, 256)
    input_intrinsics = torch.randn(1, 3, 3)
    input_sc_mask = torch.ones(1, 1, 256, 256)
    input_random_rescale_sc = False
    input_sample_level = 3
    pose = transformer_head(
        input_sc, input_intrinsics, input_sc_mask, input_random_rescale_sc, input_sample_level)
    print(pose.shape)
